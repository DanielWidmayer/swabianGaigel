...
var containerHeight = document.getElementById("card-table").offsetHeight;
var containerWidth = document.getElementById("card-table").offsetWidth;
...
lowerhand.click(function (card) {
    if (lastTrick == 1) {
        ...
        lowerPlayingPile.render({ callback: lowerPlayingPile.topCard().rotate(getRandomArbitrary(-20,20)) });
        lowerhand.render();
        setTimeout(() => {
            upperPlayingPile.addCard(upperhand.randomCard());
            upperPlayingPile.render({ callback: upperPlayingPile.topCard().rotate(getRandomArbitrary(-200, -160)) });
            upperhand.render();
            setTimeout(() => {
                if (evaluateTrick() == 0) {
                    lastTrick = 0;
